...
const createCollege = async (req, res) => {
    try {
        ...
        if (Object.keys(data) == 0) return res.status(400).send({ status: false, message: "Data is required to create college" })   // if user gives no deatils/no blanck obj , 
        ...
        if (duplicateName) { return res.status(400).send({ status: false, message: "College name already existed" }) }

        if (!isValid(fullName)) { return res.status(400).send({ status: false, message: "Full name is required" }) }
        if (!isValidLogoLink(logoLink)) { return res.status(400).send({ status: false, message: "Logo link is required" }) }
...
const getColleges = async (req, res) => {
    ...
    try {
        ...
        if (collegeId.length == 0) { return res.status(404).send({ status: false, message: "Please enter a valid name abbreviation in lowercase" }) }
